        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="选择分类" prop="catalogId">
              <category-cascader ref="CategoryCascader"></category-cascader>
            </el-form-item>
            <el-form-item label="选择品牌" prop="brandId">
              <brand-select ref="BrandSelect"></brand-select>
            </el-form-item>
...
import BrandSelect from "../common/common-brand-select";
...
export default {
  ...
  methods: {
    ...
    //清空组件CategoryCascader和BrandSelect在提交后，仍然保留选项内容的问题
    clearCascader(){
        this.$refs.CategoryCascader.refreshCascader();  //调用子组件方法
        this.$refs.BrandSelect.refreshCascader();
    },
    ...
    getShowSaleAttr() {
      if (!this.dataResp.steped[1]) {
        this.$http({
          ...
          this.dataResp.saleAttrs = data.attrListPage.list;
          data.attrListPage.list.forEach(item => {
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$http({
          ...
          //循环获取商品分组VO列表
          data.AttrGroupVOList.forEach(item => {
            let attrArray = [];
            if (item.attrs != null) {
               //循环获取规格参数列表，保存到baseAttrs
            ...
            this.dataResp.baseAttrs.push(attrArray);     //保存属性列表
          });
          this.dataResp.steped[0] = 0;
          this.dataResp.attrGroups = data.AttrGroupVOList;   //保存商品分组VO列表
        });
      }
    },
    //提交SKU输入
