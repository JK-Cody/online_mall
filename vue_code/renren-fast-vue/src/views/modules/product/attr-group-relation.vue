      ...
      <el-dialog width="40%" title="选择属性" :visible.sync="innerVisible" append-to-body>
        <div>
          <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
            <el-form-item>
              <el-input v-model="dataForm.key" placeholder="请输入属性名或id" clearable></el-input>
            </el-form-item>
            <el-form-item>
      ...
      <el-row>
        <el-col :span="24">
          ...
          >
            ...
            <el-table-column prop="attrId" label="序号"></el-table-column>
  ...
  methods: {
    ...
    submitAddRealtion() {
      ...
      if (this.innerdataListSelections.length > 0) {
        ...
        this.$http({
          ...
          this.$emit("refreshData");  //给接收页面发送一个事件，携带上数据
          this.init(this.attrGroupId);
        });
      } else {
      }
    ...
    getDataList() {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          this.dataList = data.AttrListWithoutRelation.list;
          this.totalPage = data.AttrListWithoutRelation.totalCount;
        } else {
          this.dataList = [];
          this.totalPage = 0;
        }
