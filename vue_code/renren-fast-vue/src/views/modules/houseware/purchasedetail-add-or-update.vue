  ...
  >
    ...
    >
      <el-form-item label="商品ID" prop="skuId">
        <el-input v-model="dataForm.skuId" placeholder="采购商品的skuId"></el-input>
      </el-form-item>
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/houseware/purchasedetail/info/${this.dataForm.id}`
            ),
            method: "get",
            params: this.$http.adornParams()
          }).then(({ data }) => {
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/houseware/purchasedetail/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
