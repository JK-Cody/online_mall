<template>
  <el-row :gutter="20">
    <el-col :span="6">
       <!-- 获取组件Category的事件 -->
      <category @listChildren="treenodeclick"></category>
    </el-col>
    <el-col :span="18">
      <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getAttrList()">
          ...
          <el-form-item>
            ...
            <el-button type="success" @click="getAllAttrList()">查询全部</el-button>
            ...
            <el-button
              ...
              type="danger"
              @click="deleteHandle()"
              :disabled="attrListSelections.length <= 0"
            >批量删除</el-button>
          </el-form-item>
        </el-form>
        <el-table
        ...
        >
          ...
          <el-table-column prop="enable" header-align="center" align="center" label="启用" width="50px">
            <template slot-scope="scope">
...
import Category from "../common/common-category";
...
export default {
  ...
  methods: {
    ...
    getAttrList() {
      ...
      this.$http({
        //发送 sale/list/{catalogId} 或 /base/list/{catalogId} 请求
        ...
        if (data && data.code === 0) {
          this.attrList = data.attrListPage.list;
          this.totalPage = data.attrListPage.totalCount;
        } else {
          this.attrList = [];
          this.totalPage = 0;
        }
    ...
    deleteHandle(id) {
      var ids = id
        ? [id]
        : this.attrListSelections.map(item => {
            return item.attrId;
          });
      this.$confirm(
